...
```{r}
analysis_dir <- "/R/Shared/CPDB_Vis/"
setwd(analysis_dir)

#change to wherever the seurat object is. Here it is referencing one directory above analysis_dir.
...
pvals_he <- read.delim("MC_v8_imm_byCohort/imm_he_cpdb/statistical_analysis_pvalues.txt", check.names = FALSE)
means_he <- read.delim("MC_v8_imm_byCohort/imm_he_cpdb/statistical_analysis_means.txt", check.names = FALSE)
...
pvals_he_all <- read.delim("MC_v8_all_byCohort/all_he_cpdb/statistical_analysis_pvalues.txt", check.names = FALSE)
means_he_all <- read.delim("MC_v8_all_byCohort/all_he_cpdb/statistical_analysis_means.txt", check.names = FALSE)
...
pvals_he_all_format <- format_for_cpdb(pvals_he_all, gene_mod)
means_he_all_format <- format_for_cpdb(means_he_all, gene_mod)
...
pvals_he_all_format <- replace_name_cpdb(pvals_he_all_format, "Enterocyte", "Ent.")
means_he_all_format <- replace_name_cpdb(means_he_all_format, "Enterocyte", "Ent.")
pvals_he_all_format <- replace_name_cpdb(pvals_he_all_format, "Cycling", "Cyc.")
means_he_all_format <- replace_name_cpdb(means_he_all_format, "Cycling", "Cyc.")
...
he <- plot_cpdb(
    ...
    celltype_key="ident_L3_format",
    genes=c("CXCL11|CD226"),
    means=means_he_all_format,
    pvals=pvals_he_all_format,
    keep_significant_only = FALSE,
    min_interaction_score=0.25
)
...
pdf(paste(fig_dir, "SuppFig8_CXCL11_CD226_ByCohort_NameAbbrev.pdf", sep="/"), width = 14, height = 7)
mc_pan / cd_pan / he_pan + plot_layout(guides="collect")
dev.off()
...
pdf(paste(fig_dir, "SuppFig8_IntPlot_all.pdf", sep="/"), width = 15, height = 18)
print(tmp_plot)
dev.off()
```
...
pdf(paste(fig_dir, "SuppFig9_IntPlot_imm.pdf", sep="/"), width = 13, height = 13)
print(tmp_plot)
dev.off()
```
...
pvals_he_format <- format_for_cpdb(pvals_he, gene_mod)
means_he_format <- format_for_cpdb(means_he, gene_mod)
...
he_imm <- plot_cpdb(
    ...
    celltype_key="ident_L3_format",
    gene_family="chemokine",
    means=means_he_format,
    pvals=pvals_he_format
    )
...
#healthy has no significant int.
...
pdf(paste(fig_dir, "SuppFig10_MC.pdf", sep="/"), width = 12, height = 18)
mc_ifn_pan / cd_ifn_pan / mc_pan / cd_pan / he_pan + mc_imm_co_pan + plot_layout(guides="collect", heights=c(0.5, 0.5, 2, 1, 1, 8))
dev.off()
```
